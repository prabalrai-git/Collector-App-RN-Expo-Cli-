import {
  GetCurrentLocationOfCollector,
  GetListOfCollectors,
  GetlocationofCollectorByDate,
  InsertupdateCollectorLocationDetails,
  ReassignCollectorToRejectedSample,
} from "../constants/url";
  ...
  return async (dispatch) => {
    try {
      const response = await fetch(
        `${GetlocationofCollectorByDate}?entrydate=${data.entrydate}&userId=${data.userId}`
      );
      if (response?.status === 200) {
        sucessCallback(response?.data);
      } else {
...
export const GetCurrentLocationOfuser = (data, sucessCallback) => {
  return async (dispatch) => {
    ...
    try {
      const response = await fetch(
        `${GetCurrentLocationOfCollector}?userId=${data}`
      );
      if (response?.status === 200) {
        sucessCallback(response?.data);
      } else {
...
export const ReassignCollectorToCollector = (data, sucessCallback) => {
  return async (dispatch) => {
    try {
      const response = await store(
        `${ReassignCollectorToRejectedSample}?userId=${data.userId}&requestId=${data.requestId}&collectorId=${data.collectorId}&reqStatus=${data.reqStatus}&sampleStatus=${data.sampleStatus}&remarks=${data.remarks}`
      );
      if (response?.status === 200) {
        sucessCallback(response?.data);
      } else {
