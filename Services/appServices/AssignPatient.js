import {
  AssignCollectorForSampleCollection,
  GetAddressOfClientByClientId,
  GetCollectionRequestHistoryByPatientId,
  GetCollectionRequestPatientDetails,
  GetCollectorRequestByCollectorWiseForWeekWithStatus,
  GetHomeCollectionTestRequestTestListByRequestId,
  GetMostPopularTestList,
  GetReferredDoctorListForCollector,
  GetRequestorForCollection,
  GetSampleRequestListByCollectorIdAndDateRange,
  GetSampleRequestStatus,
  GetTestListforHomeCollection,
  InsertUpdateHomeCollectionRequest,
...
export const AssignPatient = (data, returnData) => {
  return async (dispatch) => {
    try {
      ...
      // console.log(data);
      const response = await storeNested(
        AssignCollectorForSampleCollection,
        JSON.stringify(data)
      );
  ...
  return async (dispatch) => {
    try {
      const response = await fetch(
        `${GetCollectionRequestPatientDetails}Id?id=0`
      );
      if (response?.status === 200) {
...
export const GetSampleRequestListByCollector = (data, sucessCallback) => {
  ...
  return async (dispatch) => {
    try {
      const response = await fetch(
        `${GetSampleRequestListByCollectorIdAndDateRange}?collectorId=${data.collectorId}&fromdate=${data.fromDate}&todate=${data.toDate}`
      );
      if (response?.status === 200) {
        sucessCallback(response?.data);
      } else {
...
export const GetHomeCollectionTestRequestTestList = (data, sucessCallback) => {
  ...
  return async (dispatch) => {
    try {
      const response = await fetch(
        `${GetHomeCollectionTestRequestTestListByRequestId}?requestId=${data}`
      );
      if (response?.status === 200) {
        sucessCallback(response?.data);
      } else {
...
export const GetAddressOfClient = (data, sucessCallback) => {
  return async (dispatch) => {
    try {
      const response = await fetch(
        `${GetAddressOfClientByClientId}?clientId=${data}`
      );
      if (response?.status === 200) {
...
export const GetCollectorRequestByCollectorWiseForWeek = (
  data,
  sucessCallback
) => {
  return async (dispatch) => {
    try {
      const response = await fetch(
        `${GetCollectorRequestByCollectorWiseForWeekWithStatus}?collectorId=${data}`
      );
      if (response?.status === 200) {
        // console.log('data found', response?.data);
        sucessCallback(response?.data);
...
export const GetCollectionRequestHistory = (data, sucessCallback) => {
  ...
  return async (dispatch) => {
    try {
      ...
      const response = await fetch(
        `${GetCollectionRequestHistoryByPatientId}?patid=${data.patid}&collectorId=${data.collectorId}`
      );
  ...
  return async (dispatch) => {
    try {
      const response = await store(
        `${UpdateIsPaidStatusByCollector}?userId=${data.userId}&requestId=${data.requestId}&ispaid=${data.ispaid}&remarks=${data.remarks}`
      );
