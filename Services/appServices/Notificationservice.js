import {
  GetcollectorNotificationByUserId,
  GetSampleRequestDetailsByRequestId,
  InsertUpdateNotificationDetails,
  UpdateNotificationSeenFlag,
} from "../constants/url";
  ...
  return async (dispatch) => {
    try {
      const response = await fetch(
        `${GetcollectorNotificationByUserId}?uId=${data}`
      );
...
export const GetSampleRequestDetailsByRId = (data, sucessCallback) => {
  return async (dispatch) => {
    try {
      const response = await fetch(
        `${GetSampleRequestDetailsByRequestId}?rId=${data}`
      );
      if (response?.status === 200) {
  ...
  return async (dispatch) => {
    try {
      const response = await store(
        `${UpdateNotificationSeenFlag}?notificationId=${data}&isSeen=true`
      );
      if (response?.status === 200) {
